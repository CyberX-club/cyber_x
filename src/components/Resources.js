      ...
      >
        {selectedResource && (
          <Fade in={open} timeout={600}>
            <Card
              sx={{ maxWidth: 700, backgroundColor: "#1a1a1a", color: "#fff" }}
            >
              <DialogTitle>
                <Typography
                  variant={"h5"}
              ...
              <DialogContent>
                {selectedResource.img && (
                  <CardMedia
                    ...
                    onError={(e) => {
                      console.error(
                        `Failed to load image in dialog: ${selectedResource.img}`
                      );
                      e.target.onerror = null;
                      e.target.src = "/resources/ser-bg.jpeg";
                    }}
                ...
                {selectedResource.postedAt && (
                  <Typography variant="body2" sx={{ color: "#0f0" }}>
                    Posted at:{" "}
                    {new Date(selectedResource.postedAt).toLocaleDateString()}
                  </Typography>
                )}
