...
import { Computer, Lock, Code, BugReport, Storage, Memory } from '@mui/icons-material';

const MagazineEntry = ({ title, author, images = [], text = '', imagePositions = [] }) => {
    ...
    paragraphs.forEach((paragraph, index) => {
        content.push(
            <Typography
                ...
                sx={{ marginBottom: 2, color: theme.palette.text.primary, lineHeight: 1.8 }}
            >
                {paragraph}
            </Typography>
        );
    ...
    return (
        <Card sx={{
            ...
            <CardContent sx={{ position: 'relative', zIndex: 1, maxWidth: '800px', margin: 'auto' }}>
                {/* Title */}
                <Typography
                    variant="h3"
                    component="h1"
